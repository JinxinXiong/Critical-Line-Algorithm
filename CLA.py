...
class CLA:
        ...
        while True:
            ...
            if len(f) < self.mean.shape[0]:
                ...
                for i in b:
                    ...
                    if (self.l[-1] is None or l < self.l[-1]) and (l_out is None or l > l_out):
                        l_out, i_out = l, i

            # When the next is negative or it cannot be computed, no additional turning points can be derived.
            # characterized by lambda = 0 and the vector mu_F is zeros
            if (l_in is None or l_in < 0) and (l_out is None or l_out < 0):
                ...
                # 4) decide lambda
                # choose the larger lambda and update F and B
                if l_in is not None and l_in > l_out:
                    self.l.append(l_in)
                    f.remove(i_in)
    ...
    def initAlgo(self):
        ...
        # fill
        a['id'] = range(self.mean.shape[0])
        a['mu'] = b

        # 2) Sort structured array
        b = np.sort(a, order='mu')  # increasing order
        # 3) First free weight
        i, w = b.shape[0], np.copy(self.lB)  # start from the last place (i.e. the largest mean return to the smallest)
        while sum(w) < 1:
            i -= 1
            w[b['id'][i]] = self.uB[b['id'][i]]
        w[b['id'][i]] += 1-sum(w)
        return [b['id'][i]], w
    ...
    # ---------------------------------------------------------------
    def computeLambda(self, covarF_inv, covarFB, meanF, wB, j, bi):
        # j represent the jth element for the w_F and mu_F
        # in bi the i should be the corresponding index in the entire weight w
        # 1) C
        onesF = np.ones(meanF.shape)
        ...
        if wB is None:
            ...
            return float(((1 - l1 + l2) * c4[j] - c1 * (bi + l3[j])) / c), bi
    ...
    def reduceMatrix(self, matrix, listX, listY):
        ...
        # listX, listY are the slicing index for rows and columns
    ...
    def goldenSection(self, obj, a, b, **kargs):
        ...
        # obj: the target function
        # a, b are the boundary for the delimited region for finding the minimum
